...
const V1 = '';
// 获取首页项目列表
export const getDataListService = () => {
...
// 根据id新增项目
export async function addParamsService(params: any) {
  return fetchPost(`${V1}project`, { body: params });
};
// 根据id获取项目
export async function getParamsService(id: string) {
  return fetchGet(`${V1}project/${id}`);
};
// 根据id修改项目
export async function updateParamsService(id: string, params?: any) {
  return fetchPut(`${V1}project/${id}`, { body: params });
};
// 根据id删除项目
export async function deleteParamsService(id: string) {
...
// 根据id获取当前任务的历史记录列表
export const getHistoryListService = (id: string) => {
  return fetchGet(`${V1}/project_history/${id}`);
};
// 获取案例库列表
export const getCaseListService = () => {
  return fetchGet(`${V1}/case_list`);
};
// 流程图-流程测试-运行流程到此处
export async function processTestService(params: object) {
  return fetchPost(`${V1}process_test`, { body: params });
};
// 流程图-单元测试-单节点运行
export async function unitTestService(params: object) {
